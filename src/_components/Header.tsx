  ...
  return (
    <header className="w-full">
      <div className="w-full z-10 fixed top-0 left-0 right-0 h-[82px] bg-dark flex items-center justify-between px-4 lg:px-[78px] ">
        <Image src={'/images/logo.svg'} alt="Myxellia-logo" width={153.23} height={26} style={{ width: 'auto' }}
        ...
        <div className="flex items-center gap-2 md:gap-[25px] ">
          <nav className="flex items-center gap-1 md:gap-6">
            {
              isDrawerOpen && headerNavItems.map((item) => (
                <button onClick={()=>handleClose(item.name)} key={item.name} disabled={item.isDisabled} className="w-8 h-8 grid place-content-center nav_items">
                  {item.img}
                </button>
              ))
            }
